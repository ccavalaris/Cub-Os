import { PrismaClient, type Priority, type TaskStatus, type InteractionSource, type EventStatus } from "@prisma/client";

const prisma = new PrismaClient();

/// Demo data for a fictional club. Dates are all relative to the day the seed
/// runs, so the dashboard is never showing a stale week — an event seeded "5
/// days out" is still 5 days out whenever the demo happens.
...
async function main() {
  ...
  console.log(`Seeded ${course.name}:`, counts);
}
